...
def environ(monkeypatch, request):
    ...
    if request.param == "environ":
        ...
        env.read_envfile("src/tests/envfile")
...
def test_list():
    ...
    assert_type_value(list, list_int, env.list("LIST_INT_WITH_SPACES", subcast=int))
...
def test_dict():
    ...
    assert_type_value(dict, dict_int, env("DICT_INT", cast=dict, subcast=int))
...
def test_preprocessor():
    assert_type_value(str, "FOO", env("STR", preprocessor=lambda v: v.upper()))
...
def test_postprocessor(monkeypatch):
    ...
    assert_type_value(dict, expected, env.url("redis_url", postprocessor=django_redis))
...
def test_schema():
    ...
    assert_type_value(
        str, "manual_default", env("STR_DEFAULT", default="manual_default")
    )
